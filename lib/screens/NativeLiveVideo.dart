...
startLoading(context){
  return showDialog(context: context,barrierDismissible: false,builder: (context){
    return AlertDialog(content: Container(height: 200,child: Center(child: CircularProgressIndicator(),),),);
  });
}
...
getLiveVideo() async{
  ...
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: Text('Live Videos'),),body: 
    FutureBuilder(future: getLiveVideo(), builder: (BuildContext context, AsyncSnapshot snapshot) {
      if(snapshot.data != null){
        ...
        return ListView.builder(itemCount:videos.length, itemBuilder: (BuildContext context, int index) {
          return ListTile(title: Text(videos[index]['teacher']['name']),leading: Icon(Icons.live_tv),onTap: () async{
            await onJoin(videos[index]['teacher']['username'],videos[index]['id']);
          },);
        },);
        }
